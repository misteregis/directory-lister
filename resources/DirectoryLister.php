 ...
 */
class DirectoryLister extends HZip {
    ...
    */
    public function zipDirectory($directory) {
        if ($this->_config['zip_dirs']) {
            ...
            $zipcreated = "{$filename_no_ext}.zip";

            // TODO: Probably we have to parse exclude list more carefully
            $exclude_list = $this->_config['hidden_files'];

            $this->zipDir($directory, array('zipname' => $zipcreated), $exclude_list);
        }
    }
    ...
    /**
     * Returns the path to the chosen assets directory
     *
     * @param bool $absolute Whether or not the path returned is absolute (default = false).
     * @return string Path to theme
     * @access public
     */
    public function getAssetsPath($absolute = false) {
        if ($absolute) {
            // Set the assets path
            $assetsPath = $this->_appDir . '/assets';
        } else {
            // Get relative path to application dir
            $realtivePath = $this->_getRelativePath(getcwd(), $this->_appDir);

            // Set the assets path
            $assetsPath = $realtivePath . '/assets';
        }
     ...
     * @access public
     */
    function getFileSize($filePath, $_bytes = true) {

        // Get file size
        if (is_file($filePath))
            $bytes = get_filesize($filePath);
        else
            $bytes = $filePath;

        if ($_bytes) return $bytes;
    ...
    public function getFileHash($filePath) {
        ...
        if ($this->getFileSize($filePath) > $this->_config['hash_size_limit']) {

            // Notify user that file is too large
            $hashArray['md5']  = '[ O tamanho do arquivo excede o limite de ' . $this->getFileSize($this->_config['hash_size_limit'], false) . ']';
            $hashArray['sha1'] = '[ O tamanho do arquivo excede o limite de ' . $this->getFileSize($this->_config['hash_size_limit'], false) . ']';
            $hashArray['sha256'] = '[ O tamanho do arquivo excede o limite de ' . $this->getFileSize($this->_config['hash_size_limit'], false) . ']';
    ...
    protected function _setDirectoryPath($dir) {
        ...
        if (!file_exists($dir) || !is_dir($dir)) {
            // Set the error message
            $this->setSystemMessage('danger', '<b>ERRO:</b> O caminho do arquivo não existe');
        ...
        if ($this->_isHidden($dir)) {
            // Set the error message
            $this->setSystemMessage('danger', '<b>ERRO:</b> Acesso negado');
        ...
        || strpos($dir, '..') !== false || strpos($dir, '/') === 0) {
            // Set the error message
            $this->setSystemMessage('danger', '<b>ERRO:</b> Um caminho inválido foi detectado');
    ...
    protected function _readDirectory($directory, $sort = 'natcase') {
        ...
        foreach ($files as $file) {
            ...
            if ($file != '.') {
                ...
                $relativePath = $directory . '/' . urldecode($file);
                ...
                if ($file == '..') {
                    ...
                    if ($this->_directory != '.' || $file != 'index.php') {
                        // Build the file path
                        $urlPath = implode('/', array_map('rawurlencode', explode('/', utf8_encode($relativePath))));

                        if (is_dir($relativePath)) {
                            $urlPath = $this->containsIndex($relativePath) ? $relativePath : '?dir=' . str_replace('%20', '+', $urlPath);//$urlPath;
                        }
                        ...
                        $directoryArray[pathinfo($relativePath, PATHINFO_BASENAME)] = array(
                            ...
                            'url_path'   => $urlPath,
                            'file_size'  => is_dir($realPath) ? '-' : $this->getFileSize($realPath, false),
                            'mod_time'   => date($this->_config['date_format'], filemtime($realPath)),
                            'icon_class' => $iconClass,
                            'sort'       => $sort,
                            'is_file'    => is_file($relativePath) === true
                        );
                    }
